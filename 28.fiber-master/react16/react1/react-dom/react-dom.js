...
// let PlacementAndUpdate = 'PlacementAndUpdate' // 表示又要插入又要更新 比如说某个dom属性变了同时还要和某个dom交换位置
let Snapshot = 'Snapshot' // 新周期
let PlacementAndUpdateAndDeletion = 'Placement|Update|Deletion' // 表示所有变化的总和
// react中是用的 ~ | & 这种位运算来给fiber做标记的
...
// HostPoratl ContextProvider ContextConsumer Fragment FunctionComponent
...
class FiberNode {
  constructor(tag, pendingProps, key) {
    ...
    // effect怎么挂 你觉得怎么挂好 是每生成(或发现有更新)一个新的fiber就
    // 往上挂一个好 还是说咱们从下往上挂好？
...
function reconcileChildFiber(workInProgress, nextChildren) {
  ...
  // 也有可能是一个数组
  // 还可能就是个文本类型
  if (nextChildren instanceof Object && !!nextChildren.$$typeof) {
...
function updateClassComponent(workInProgress) {
  ...
  if (!!component.defaultProps) {
    nextProps = Object.assign({}, type.defaultProps, nextProps)
  }
...
function completeWork(workInProgress) {
  ...
  if (tag === HostComponent) {
    ...
    if (!instance) {
      let instance = document.createTextNode(newText)
      workInProgress.stateNode = instance
    } else {
      if (oldText !== newText) {
...
function completeUnitOfWork(workInProgress) {
  ...
  while(true) {
    ...
    let effectTag = workInProgress.effectTag
    let hasChange = PlacementAndUpdateAndDeletion.includes(effectTag)
    if (hasChange) {
      if (!!returnFiber.lastEffect) {
  ...
  while (!!nextEffect) {
    ...
    if (effectTag.includes(Snapshot)) {
      if (nextEffect.tag === ClassComponent) {
        ...
        let snapshot = instance.getSnapshotBrforeUpdate(prevProps, prevState) || {}
